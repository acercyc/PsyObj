function bounds=TextBounds_unicode(w,text)
% modify the original TextBound function for unicode text
% 1.0 - Acer 2015/02/03 05:29
...
if iscell(text)
	for i=1:length(text)
		string=char(text(i));
        Screen('DrawText',w,string,0,0,255, [], 0);
    end
else
    % --------------------------- modify by Acer ---------------------------
    % for i=1:size(text,1)
    ...
    %     Screen('DrawText',w,string,0,0,255, [], 0);
    % end
    Screen('DrawText',w, text,0,0,255, [], 0);
    % ----------------------------------------------------------------------
end
